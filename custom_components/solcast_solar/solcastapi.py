...
class SolcastApi:
    ...
    def __init__(
        ...
        self._data = {
            "siteinfo": {},
            "last_updated": dt.fromtimestamp(0, timezone.utc).isoformat(),
        }
        self._api_used = 0
        self._api_limit = 0
    ...
    async def serialize_data(self):
        ...
        if not self._loaded_data:
            _LOGGER.debug(
                "SOLCAST - serialize_data not saving data as it has not been loaded yet"
            )
            return
    ...
    async def sites_data(self):
        ...
        try:
            ...
            for spl in sp:
                ...
                _LOGGER.debug(
                    f"SOLCAST - trying to connect to - {self.options.host}/rooftop_sites?format=json&api_key=REDACTED"
                )
                async with async_timeout.timeout(60):
                    ...
                    _LOGGER.debug(
                        f"SOLCAST - sites_data code http_session returned data type is {type(resp_json)}"
                    )
                    _LOGGER.debug(
                        f"SOLCAST - sites_data code http_session returned status {status}"
                    )
                ...
                if status == 200:
                    ...
                    raise Exception(
                        "SOLCAST - HTTP sites_data error: Solcast Error gathering rooftop sites data."
                    )
        except json.decoder.JSONDecodeError:
            _LOGGER.error(
                "SOLCAST - sites_data JSONDecodeError.. The data returned from Solcast is unknown, Solcast site could be having problems"
            )
            ...
            _LOGGER.error(
                "SOLCAST - sites_data TimeoutError Error - Timed out connection to solcast server"
            )
        except Exception:
            _LOGGER.error(
                "SOLCAST - sites_data Exception error: %s", traceback.format_exc()
            )
    ...
    async def sites_usage(self):
        ...
        try:
            ...
            params = {"api_key": sp[0]}
            _LOGGER.debug("SOLCAST - getting usage data from solcast")
            async with async_timeout.timeout(60):
                resp: ClientResponse = await self.aiohttp_session.get(
                    url="https://api.solcast.com.au/json/reply/GetUserUsageAllowance",
                    params=params,
                    ssl=False,
                )
                resp_json = await resp.json(content_type=None)
            ...
            _LOGGER.error(
                "SOLCAST - sites_usage JSONDecodeError.. The data returned from Solcast is unknown, Solcast site could be having problems"
            )
        except ConnectionRefusedError as err:
            ...
            if len(self._sites) > 0:
                if file_exists(self._filename):
                    with open(self._filename) as data_file:
                        ...
                        _LOGGER.debug(
                            f"SOLCAST - load_saved_data file exists.. file type is {type(jsonData)}"
                        )
                        if json_version == _JSON_VERSION:
                            ...
                            if len(ks.keys()) > 0:
                                ...
                                _LOGGER.debug(
                                    "SOLCAST - Must be new API jey added so go and get the data for it"
                                )
                                for a in ks:
                                    await self.http_data_call(
                                        r_id=a, api=ks[a], dopast=True
                                    )
                            ...
                            for s in jsonData["siteinfo"]:
                                if not any(
                                    d.get("resource_id", "") == s for d in self._sites
                                ):
                                    _LOGGER.info(
                                        f"Solcast rooftop resource id {s} no longer part of your system.. removing saved data from cached file"
                                    )
                                    l.append(s)
    ...
    async def delete_solcast_file(self, *args):
        _LOGGER.debug("SOLCAST - service event to delete old solcast.json file")
        try:
            if file_exists(self._filename):
            ...
            _LOGGER.error(
                "SOLCAST - service event to delete old solcast.json file failed"
            )
    ...
    async def get_forecast_list(self, *args):
        try:
            ...
            _LOGGER.error("SOLCAST - service event to get list of forecasts failed")
            return None
    ...
    def get_forecast_day(self, futureday) -> dict[str, Any]:
        ...
        tup = tuple({**d, "period_start": d["period_start"].astimezone(tz)} for d in h)
        ...
        for index in range(0, len(tup), 2):
            if len(tup) > 0:
                try:
                    x1 = round(
                        (tup[index]["pv_estimate"] + tup[index + 1]["pv_estimate"]) / 2,
                        4,
                    )
                    x2 = round(
                        (tup[index]["pv_estimate10"] + tup[index + 1]["pv_estimate10"])
                        / 2,
                        4,
                    )
                    x3 = round(
                        (tup[index]["pv_estimate90"] + tup[index + 1]["pv_estimate90"])
                        / 2,
                        4,
                    )
                    hourlyturp.append(
                        {
                            "period_start": tup[index]["period_start"],
                            "pv_estimate": x1,
                            "pv_estimate10": x2,
                            "pv_estimate90": x3,
                        }
    ...
    def get_forecast_n_hour(self, hourincrement) -> int:
        ...
        try:
            ...
            g = tuple(
                d
                for d in self._data_forecasts
                if d["period_start"] >= da
                and d["period_start"] < da + timedelta(hours=1)
            )
    ...
    def get_forecast_custom_hour(self, hourincrement) -> int:
        ...
        try:
            danow = dt.now(timezone.utc).replace(minute=0, second=0, microsecond=0)
            da = dt.now(timezone.utc).replace(
                minute=0, second=0, microsecond=0
            ) + timedelta(hours=hourincrement)
            g = []
    ...
    def get_power_production_n_mins(self, minuteincrement) -> float:
        ...
        try:
            ...
            m = min(
                (z for z in self._data_forecasts),
                key=lambda x: abs(x["period_start"] - da),
            )
    ...
    def get_remaining_today(self) -> float:
        ...
        try:
            ...
            g = tuple(
                d
                for d in self._data_forecasts
                if d["period_start"].astimezone(tz).date() == da.date()
                and d["period_start"].astimezone(tz) >= da
            )
    ...
    async def http_data_call(self, r_id=None, api=None, dopast=False):
        ...
        if dopast:
            ae = None
            resp_dict = await self.fetch_data(
                "estimated_actuals", 168, site=r_id, apikey=api, cachedname="actuals"
            )
            if not isinstance(resp_dict, dict):
                _LOGGER.warning(
                    "SOLCAST - No data was returned so this WILL cause errors.. either your limit is up, internet down.. what ever the case is it is NOT a problem with the integration, and all other problems of sensor values being wrong will be a seen"
                )
            ...
            oldest = dt.now(self._tz).replace(
        ...
        resp_dict = await self.fetch_data(
            "forecasts", 168, site=r_id, apikey=api, cachedname="forecasts"
        )
        if not isinstance(resp_dict, dict):
            raise TypeError(f"resp_dict must be a dict, not {type(resp_dict)}")
        ...
        for x in _data:
            ...
            itm = next(
                (
                    ...
                    if item["period_start"] == x["period_start"]
                ),
                None,
            )
            if itm:
    ...
    async def fetch_data(
        ...
        try:
            ...
            async with async_timeout.timeout(120):
                ...
                if self.apiCacheEnabled and file_exists(apiCacheFileName):
                    _LOGGER.debug(
                        f"SOLCAST - Getting cached testing data for site {site}"
                    )
                    status = 404
                    with open(apiCacheFileName) as f:
                        resp_json = json.load(f)
                    ...
                    if status == 200:
                        _LOGGER.debug(
                            f"SOLCAST - API returned data. API Counter incremented from {self._api_used} to {self._api_used + 1}"
                        )
                    else:
                        _LOGGER.warning(
                            f"SOLCAST - API returned status {status}. API data  {self._api_used} to {self._api_used + 1}"
                        )
                        _LOGGER.warning(
                            "This is an error with the data returned from Solcast, not the integration!"
                        )
                ...
                _LOGGER.debug(
                    f"SOLCAST - fetch_data code http_session returned data type is {type(resp_json)}"
                )
                _LOGGER.debug(
                    f"SOLCAST - fetch_data code http_session status is {status}"
                )
            ...
            if status == 429:
                ...
                _LOGGER.warning(
                    "SOLCAST - Error 404. The rooftop site cannot be found or is not accessible."
                )
            ...
            for s in self._data["siteinfo"]:
                ...
                for x in self._data["siteinfo"][s]["forecasts"]:
                    ...
                    if zz.date() < lastday and zz.date() > yesterday:
                        ...
                        itm = next(
                            (item for item in _forecasts if item["period_start"] == z),
                            None,
                        )
                        if itm:
                            itm["pv_estimate"] = round(
                                itm["pv_estimate"] + (x["pv_estimate"] * self._damp[h]),
                                4,
                            )
                            itm["pv_estimate10"] = round(
                                itm["pv_estimate10"]
                                + (x["pv_estimate10"] * self._damp[h]),
                                4,
                            )
                            itm["pv_estimate90"] = round(
                                itm["pv_estimate90"]
                                + (x["pv_estimate90"] * self._damp[h]),
                                4,
                            )
                        else:
                            _forecasts.append(
                                {
                                    "period_start": z,
                                    "pv_estimate": round(
                                        (x["pv_estimate"] * self._damp[h]), 4
                                    ),
                                    "pv_estimate10": round(
                                        (x["pv_estimate10"] * self._damp[h]), 4
                                    ),
                                    "pv_estimate90": round(
                                        (x["pv_estimate90"] * self._damp[h]), 4
                                    ),
                                }
                            )
    ...
    async def checkDataRecords(self):
        ...
        for i in range(0, 6):
            ...
            if len(h) == 48:
                ...
                _LOGGER.debug(
                    f"SOLCAST - Data for {da} contains only {len(h)} of 48 records and may produce inaccurate forecast data"
                )
